...
def corrupt_mnist():
    ...
    for i in range(5):
        train_images.append(torch.load(f'corruptmnist/train_images_{i}.pt'))
        train_target.append(torch.load(f'corruptmnist/train_target_{i}.pt'))
    train_images = torch.cat(train_images)
    train_target = torch.cat(train_target)
    
    test_images = torch.load('corruptmnist/test_images.pt')
    test_target = torch.load('corruptmnist/test_target.pt')
